...
RUN adduser --disabled-password $USERNAME --home /app && chown -R $USERNAME /app && \
    apk add --no-cache libpq-dev openldap-dev mariadb-dev libsasl build-base linux-headers git
...
RUN adduser --disabled-password $USERNAME --home /app && chown -R $USERNAME /app && \
    apk add --no-cache ca-certificates tzdata run-parts tmux postgresql-client mariadb-client mariadb-connector-c libldap gettext git graphviz procps socat
